...
contract PNSResolver {
    ...
    event MultihashChanged(bytes32 indexed node, string key, bytes hash);
    ...
    /**
     * Constructor.
     * @param pnsAddr The PNS registrar contract.
     */
    function PNSResolver(PNS pnsAddr) public {
        pns = pnsAddr;
    }
    ...
    /**
     * Sets the content hash associated with an PNS node.
     * May only be called by the owner of that node in the PNS registry.
    ...
    /**
     * Returns the content hash associated with an PNS node.
     * Note that this resource type is not standardized, and will likely change
     * in future to a resource type based on multihash.
     * @param node The PNS node to query.
     ...
     */
    function multihash(string key, bytes32 node) public view returns (bytes) {
